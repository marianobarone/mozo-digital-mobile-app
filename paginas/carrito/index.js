...
export default function Carrito({ navigation, route }) {
  ...
  const VisibleItem = (props) => {
    ...
    const cambiarCantidad = function (cantidad) {
      ...
      carritoPivot.find((itemFind) => itemFind._id == data.item._id).cantidad = cantidad;
  ...
  const deleteRow = (rowMap, rowKey) => {
    ...
    const prevIndex = context.carritoItems.findIndex((item) => item._id === rowKey);
  ...
  function devolverTotal() {
    ...
    context.carritoItems.forEach((carritoItem) => {total = total + carritoItem.precio * carritoItem.cantidad;});
